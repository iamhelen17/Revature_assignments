...
public class PlayerSearchDAOImpl implements PlayerSearchDAO {
	...
	public Player getPlayerByContact(String contact) throws BusinessException {
		...
		try (Connection connection = PostgresqlConnection.getConnection()) {
			...
			String sql = "select p.player_id, p.name, p.age, p.gender, p.dob, t.teamname from test2.player1 p join test2.team t on p.team_id=t.team_id where contact = ?";
	...
	public List<Player> getPlayersByGender(String gender) throws BusinessException {
		...
		try (Connection connection = PostgresqlConnection.getConnection()) {
			...
			String sql = "select p.player_id, p.name, p.age, p.contact, p.dob, t.teamname from test2.player1 p join test2.team t on p.team_id=t.team_id where gender = ?";
